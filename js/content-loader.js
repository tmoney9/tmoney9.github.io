...
async function loadProfile() {
    ...
    try {
        ...
        if (content && content.trim()) {
            parsedBio = marked.parse(content.trim()).replace(/<p>\s*~\s*<\/p>/g, '');
        } else {
            parsedBio = `
...
async function loadWorkExperience() {
    ...
    try {
        ...
        sections.forEach((section, index) => {
            ...
            const introHtml = introText ? marked.parseInline(introText.replace(/\n+/g, ' ')) : '';
            
            let bulletsListHtml = '';
            if (bulletsArray.length > 0) {
                const itemsHtml = bulletsArray.map(item => `<li>${marked.parseInline(item)}</li>`).join('');
                    ...
                    <summary class="experience-item__summary">
                        <div class="experience-item__header-top">
                            <span class="experience-item__company">
                                ${marked.parseInline(company)}
                                ${role ? ` <span class="experience-item__title-role">${marked.parseInline(role)}</span>` : ''}
                            </span>
    ...
    try {
        ...
        sections.forEach(section => {
            ...
            if (title.toLowerCase() === 'contact') {
                const contactHtml = marked.parse(body);
                html += `
    ...
    for (const thread of threadFiles) {
        ...
        try {
            ...
            renderer.paragraph = function (token) {
                const inlineText = this.parser.parseInline(token.tokens || []);
...
async function loadFullCaseStudy() {
    ...
    try {
        ...
        const parsedHtml = marked.parse(content, { renderer });
